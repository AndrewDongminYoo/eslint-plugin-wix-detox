...
const main = async () => {
  for (const ruleName of process.env.npm_config_rule
    ? process.env.npm_config_rule.split(",")
    : ruleNames) {
    ...
    let assertions;
    try {
      assertions = (await import(`./assertions/${camelCase(ruleName)}`))
        .default;
    } catch (error) {
    ...
    if (
      !("meta" in rule && "schema" in rule.meta) &&
    ...
    assertions.invalid = assertions.invalid.map((assertion) => {
      ...
      assertion.parserOptions = defaultsDeep(
        assertion.parserOptions,
        parserOptions
      );
    ...
    assertions.valid = assertions.valid.map((assertion) => {
      ...
      if (assertion.errors) {
        throw new Error(
          `Valid assertions for rule ${ruleName} should not have an \`errors\` array.`
        );
      }

      if (assertion.output) {
        throw new Error(
          `Valid assertions for rule ${ruleName} should not have an \`output\` property.`
        );
      }
  ...
  if (!process.env.npm_config_rule) {
    ...
    for (const [ruleName, assertions] of Object.entries(flatRuleNames)) {
      const rule = config.rules[ruleName];
